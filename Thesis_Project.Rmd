...
ggplot(data.frame(Actual = y_test, Predicted = pred_2024), aes(Actual, Predicted)) +
  ...
  labs(title = "Actual vs. Predicted Food Insecurity (2025)")
```
...
df <- df %>%
  mutate(rural_urban = as.factor(rural_urban),
         rural_urban = as.numeric(rural_urban),
         food_risk_score = (
      (percent_enrolled_in_free_or_reduced_lunch * 0.5) + 
      (1 - food_environment_index * 0.5)
    ),
    economic_stress = (
      percent_unemployed + 
...
ny_map <- ny_counties %>%
  ...
  scale_fill_viridis_c(
    ...
    option = "inferno", 
    direction = -1,
    limits = c(0, max(results_2025_clean$pred_2025_food_insecure))
  ) +
  labs(
    title = "New York State 2025 Predicted Food Insecurity by County",
