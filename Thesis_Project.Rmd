...
X_all <- do.call(abind, c(lapply(prepped_data$lstm_ready, function(x) x$X), along = 1))
y_all <- unlist(lapply(prepped_data$lstm_ready, function(x) x$y))

# Train-test split (time-based)
train_idx <- 1:floor(0.8 * nrow(X_all))
...
lstm_model <- build_lstm_model(dim(X_train)[2:3])
...
# Predictions -------------------------------------------------------------
# Function to predict next year's values
predict_next_year <- function(model, last_data, preproc) {
  last_scaled <- predict(preproc, last_data)
  input_data <- array_reshape(as.matrix(last_scaled), c(1, dim(last_scaled)[1], dim(last_scaled)[2]))
  pred_scaled <- model %>% predict(input_data)
  pred <- pred_scaled * preproc$std["percent_food_insecure"] + preproc$mean["percent_food_insecure"]
  return(pred)
}

# Predict 2025 values for each county
predictions_2025 <- prepped_data %>%
  mutate(
    last_3_years = map(data, ~tail(., 3)),
    prediction = map2_dbl(
      last_3_years,
      preprocessed,
      ~predict_next_year(lstm_model, select(.x, -fips), .y$preproc)
    )
  ) %>%
  select(fips, prediction)

# Create results dataframe
results_2025 <- df %>%
  filter(year == 2024) %>%
  select(fips, county.x, percent_food_insecure) %>%
  left_join(predictions_2025, by = "fips") %>%
  rename(
    current_food_insecure = percent_food_insecure,
    pred_2025_food_insecure = prediction
  ) %>%
  mutate(
    predicted_change = pred_2025_food_insecure - current_food_insecure,
    risk_category = case_when(
      pred_2025_food_insecure > quantile(pred_2025_food_insecure, 0.75, na.rm = TRUE) ~ "High Risk",
      pred_2025_food_insecure > quantile(pred_2025_food_insecure, 0.5, na.rm = TRUE) ~ "Medium Risk",
      TRUE ~ "Low Risk"
    )
  ) %>%
  arrange(desc(pred_2025_food_insecure))

# Save results
write.csv(
  results_2025,
  file = paste0("lstm_food_insecurity_predictions_", format(Sys.Date(), "%Y%m%d"), ".csv"),
  row.names = FALSE
)
...
results_2025 %>%
  ...
  labs(title = "Top 10 High-Risk Counties for 2025 Food Insecurity (LSTM)",
       x = "County",
       y = "Predicted Food Insecurity Rate") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
