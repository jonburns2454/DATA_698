...
# Filter relevant columns and remove rows with missing target
df <- data %>%
  select(-starts_with("percent_children_in_single_parent_households")) %>% # Remove duplicate columns
  select(-county.x, -state.x) %>% # Remove redundant identifiers
  drop_na(percent_food_insecure) # Remove rows with missing target
...
# Handle remaining NAs - impute with median
df <- df %>%
  mutate(across(where(is.numeric), ~ifelse(is.na(.), median(., na.rm = TRUE), .)))

# Normalize the data
preProc <- preProcess(df %>% select(-year, -fips, -percent_food_insecure), 
                    method = c("center", "scale"))

df_normalized <- predict(preProc, df) %>%
  mutate(percent_food_insecure = df$percent_food_insecure) # Keep target unnormalized
```
...
create_sequences <- function(data, n_timesteps) {
  data %>%
    arrange(fips, year) %>%
    group_by(fips) %>%
    mutate(seq_id = (row_number() - 1) %/% n_timesteps) %>%
    filter(n() >= n_timesteps) %>% # Only keep counties with enough data
    group_by(fips, seq_id) %>%
    filter(n() == n_timesteps) %>% # Only complete sequences
    ungroup() %>%
    select(-seq_id)
}

n_timesteps <- 3 # Use 3 years to predict next year
seq_data <- create_sequences(df_normalized, n_timesteps)

# Split into train (2020-2024) and test (2025)
train <- seq_data %>% filter(year < 2025)
test <- seq_data %>% filter(year == 2025)

# Modified data preparation that keeps fips for grouping
prepare_X_y <- function(data) {
  data %>%
    group_by(fips) %>%
    arrange(year) %>%
    mutate(next_year_food_insecure = lead(percent_food_insecure)) %>%
    filter(!is.na(next_year_food_insecure)) %>% # Remove last year of each county
    ungroup() # Don't drop fips here
}

train_xy <- prepare_X_y(train) %>% select(-year)
test_xy <- prepare_X_y(test) %>% select(-year)

# Separate features and target while keeping fips for grouping
X_train <- train_xy %>% select(-next_year_food_insecure)
y_train <- train_xy %>% select(fips, next_year_food_insecure)

X_test <- test_xy %>% select(-next_year_food_insecure)
y_test <- test_xy %>% select(fips, next_year_food_insecure)

reshape_for_lstm <- function(X, n_timesteps) {
  # First ensure data is properly grouped and ordered
  X <- X %>% 
    group_by(fips) %>% 
    arrange(year) %>% 
    ungroup()
  
  # Calculate number of features (excluding group columns)
  feature_cols <- setdiff(colnames(X), c("fips", "year"))
  n_features <- length(feature_cols)
  
  # Get unique counties and count sequences per county
  counties <- unique(X$fips)
  n_counties <- length(counties)
  
  # Initialize array with correct dimensions
  array_data <- array(dim = c(n_counties, n_timesteps, n_features))
  
  # Fill the array
  for (i in 1:n_counties) {
    county_df <- X %>% 
      filter(fips == counties[i]) %>% 
      select(all_of(feature_cols)) %>% 
      as.matrix()
    
    # Ensure we have exactly n_timesteps observations
    if (nrow(county_df) >= n_timesteps) {
      array_data[i,,] <- county_df[1:n_timesteps,]
    } else {
      # Handle counties with insufficient data by padding with NA
      county_df <- rbind(county_df, 
                        matrix(NA, n_timesteps - nrow(county_df), ncol(county_df)))
      array_data[i,,] <- county_df
    }
  }
...
# Then apply it:
X_train_array <- reshape_for_lstm(train, n_timesteps)  # Use original train data with fips and year
X_test_array <- reshape_for_lstm(test, n_timesteps)    # Use original test data with fips and year

# Verify the shapes
dim(X_train_array)  # Should be [n_counties, n_timesteps, n_features]
dim(X_test_array)   # Should be [n_counties, n_timesteps, n_features]

# Get target values (value after each sequence)
get_targets <- function(data, n_timesteps) {
  data %>%
    group_by(fips) %>%
    arrange(year) %>%
    mutate(seq_num = (row_number() - 1) %/% n_timesteps) %>%
    group_by(fips, seq_num) %>%
    filter(n() == n_timesteps) %>%  # Only complete sequences
    summarise(target = last(percent_food_insecure)) %>%
    pull(target)
}

y_train_array <- get_targets(train, n_timesteps)
y_test_array <- get_targets(test, n_timesteps)
...
```{r}
cat("X_train_array dimensions:", paste(dim(X_train_array), "\n"))
# Should output: [num_samples, timesteps, features] e.g., "100 3 20"
```

```{r}
library(keras)

# 1. First completely reset the environment
k_clear_session()


# 2. Rebuild your model (Functional API)
inputs <- layer_input(shape = c(3, 26))  # 3 timesteps, 26 features
outputs <- inputs %>%
...
inputs <- layer_input(shape = c(3, 26))  # timesteps × features
...
model <- keras_model(inputs, outputs, name = "food_insecurity_model")
# Check architecture
summary(model)
```
...
```{r}
# 3. COMPILE using the CORRECT syntax
model %>% keras::compile(
...
### Evaluate Model Performance and Show Predictions:
...
results <- data.frame(
  ...
  County = test %>% 
    group_by(fips) %>% 
    slice(1) %>% 
    pull(fips) # Get county IDs
)

# Calculate RMSE and MAE
postResample(predictions, y_test_array)

# Plot actual vs predicted
ggplot(results, aes(x = Actual, y = Predicted)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(title = "Actual vs Predicted Food Insecurity",
       x = "Actual Percent Food Insecure",
       y = "Predicted Percent Food Insecure") +
  theme_minimal()
```
